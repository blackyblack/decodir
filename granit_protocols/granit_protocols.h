...
//данные, полученные из NMEA как есть
typedef struct
{
  char status[2]; // Действительность данных [N]
  char latitude[12]; // Широта [GGMM.MMMM]
  char latitude_i[2]; // Северная или южная [N]
  char longitude[12]; // Долгота [GGGMM.MMMM]
  char longitude_i[2]; // Западная или восточная [N]
  char sat_used[3]; // Количество используемых спутников [N[M]]
  char hdop[8]; // Горизонтальная неточность [N.MM]
  char altitude[8]; // Высота над уровнем моря [N.M]
  char course[7]; // Курс в градусах от севера [N.MM]
  char speed[8]; // Скорость в knots [N.MM]
} gps_data_raw_struct;
